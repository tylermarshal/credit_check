...
#finds any numbers over 9 in the doubled_card_number and returns its index number
  index_of_over_9 = doubled_card_number.each_with_index.select { |num, index| num > 9 }.map { |pair| pair[1] }
#determines how many numbers are over 9 so we can iterate through and sum the digits of that many numbers
  number_of_times = index_of_over_9.length
  update_index = 0
#loops through each index number previously found in the doubled_card_number and sums its digits
  ...
  doubled_card_number[index_of_over_9[update_index]] = doubled_card_number[index_of_over_9[update_index]].to_s.chars.map(&:to_i).inject(0){|sum,x| sum + x }
  update_index += 1
  end
  doubled_card_number
end

sum_values_over_9(card_number)
